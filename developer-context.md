...
- **Smooth Scrolling**: [Lenis](https://github.com/darkroomengineering/lenis) manages the global scroll loop. Uses `ResizeObserver` on `document.body` to auto-recalculate scroll dimensions when async content loads.
...
- **Upgrade section** (logged-in users with tier < architect): Card with glow effect, gold badge "Exclusive Access", "Unlock Your Full Potential" title, perk pills (Unlimited Downloads, Premium Builds, 3D Layer-by-Layer Guides, 2D Layer-by-Layer Guides), and gold CTA button → `/pricing`.
...
- **CRITICAL: `onAuthStateChange` callback must be synchronous.** Never `await` or make Supabase client calls inside it — the callback holds an internal auth lock and making Supabase calls deadlocks the singleton client, causing ALL subsequent DB calls across the entire app to hang forever.
- **Stable `user` reference**: Uses `setUser(prev => prev?.id === newSession.user.id ? prev : newSession.user)` so the `user` object reference only changes when the actual user changes. This prevents every `useEffect([user])` across the app from re-firing on routine `TOKEN_REFRESHED` events (e.g. tab-switch recovery).
- **Profile fetch is a separate `useEffect([user])`** — reacts to user changes without blocking the auth listener.
- **`visibilitychange` recovery**: Calls `supabase.auth.getUser()` when tab becomes visible to force session recheck.
- Browser Supabase client is a **singleton** (`lib/supabase/client.ts`) with default settings.

### Middleware (`middleware.ts`)
- Standard Supabase pattern: creates server client, calls `getUser()` to refresh session, returns response with updated cookies.
- Also handles `/@handle` → `/handle` redirects.
- Runs on page routes only (skips API routes, static files, Next.js internals).

### `getUser()` vs `getSession()` Rule
- **`getUser()`** (network call, refreshes token) — used in **middleware** and **visibilitychange handler**.
- **`getSession()`** (reads cookies, no network call) — used in **server components/layouts**.
- Do not call `getUser()` in server components — middleware already handles token refresh.
...
- **Deletion requests alert**: Red banner appears on dashboard when `admin_get_deletion_requests` returns pending requests. Links to `/admin/deletion-requests` page showing users who requested account deletion with avatar, handle, email, tier, and request timestamp.
...
- **Newsletter page**: Tabbed UI with "Subscribers" (original newsletter_subscribers table) and "User Preferences" tabs. User Preferences tab shows all registered users with their `email_notifications` and `newsletter_opt_in` status via `admin_get_user_newsletter_preferences` RPC. CSV export with proper quote escaping and error handling.
- **Analytics page**: time ranges include 7d, 14d, 30d, 90d, 1 Year, Lifetime; member tier breakdown section; Page Views Over Time bar chart; Traffic Sources ("Direct" = no referrer, "Localhost:3000" = dev environment); Build Analytics search section with dropdown, stat cards, and views chart. Build view tracking uses ref guard to prevent double-counting in React strict mode.
- **Builds page**: `buildImagePlaceholder` style for products without images (prevents empty `src` warning).
- **Publish Build page**: Slug auto-generates from title as user types. Manual slug edits stop auto-generation via `slugManuallyEdited` flag.
...
### Storage Buckets
- **`build-images`**: Public bucket for build showcase images. 5MB limit, image formats only (png, jpeg, webp, gif). Public read access, admin-only write/update/delete.
- **`build-downloads`**: Private bucket for build download files. 50MB limit, archive formats (zip, rar, 7z). Authenticated read access (tier-gated in app logic), admin-only write/update/delete.
- Both buckets created via schema with RLS policies using `is_admin()` check.
...
### Architecture
- **Providers**: PayPal Subscriptions API + Stripe Checkout (both active).
- **PayPal Flow**: Pricing → Auth check → `/checkout` → PayPal SDK in-page → `/api/payments/paypal/activate-subscription` → DB update → Tier upgrade.
- **Stripe Flow**: Pricing → Auth check → `/checkout` → `/api/payments/stripe/create-checkout-session` → Stripe hosted checkout → `/api/payments/stripe/success` (redirect) → DB update → Tier upgrade. Webhook as belt-and-suspenders.
- **Server libraries**: `lib/payments/paypal.ts` (OAuth2 token caching, verification, cancellation, webhook verification), `lib/payments/stripe.ts` (lazy-initialized Stripe client, checkout session creation, cancellation, webhook verification).
- **Checkout page**: Tabbed payment UI (Card / PayPal) — Stripe is default. If only one provider is configured for a plan, that provider is shown without tabs.
...
| `POST /api/payments/paypal/cancel-subscription` | User cancellation for PayPal: calls `cancel_user_subscription` RPC, then `cancelPayPalSubscription()` |
| `POST /api/webhooks/paypal` | Receives PayPal webhook events (renewals, cancellations, etc.) |
| `POST /api/payments/stripe/create-checkout-session` | Creates Stripe Checkout Session, returns URL for redirect |
| `GET /api/payments/stripe/success` | Stripe redirects here after payment; verifies session, creates subscription + order in DB, redirects to success page |
| `POST /api/payments/stripe/cancel-subscription` | User cancellation for Stripe: calls `cancel_user_subscription` RPC, then `cancelStripeSubscription()` |
| `POST /api/webhooks/stripe` | Receives Stripe webhook events (invoice.paid, subscription.updated/deleted, etc.) |
...
| `get_checkout_plan` | Fetch plan details + PayPal/Stripe plan/price IDs for checkout page |
...
### Settings Page
- **Profile tab**: Display name, bio, Minecraft IGN with avatar preview. Handle change with 14-day cooldown.
- **Account tab**: Email (verified badge), tier card, notification preferences (email notifications + newsletter opt-in toggles tracked in DB via `update_user_preferences` RPC), danger zone with delete account button.
- **Deletion flow**: User clicks "Delete Account" → confirmation → calls `request_account_deletion` RPC → sets `deletion_requested_at` timestamp → auto signs out user. On re-login, red banner appears at top of Account tab showing "Account scheduled for deletion" with "Cancel deletion" button (calls `cancel_account_deletion` RPC).
- **Billing tab**: Fetches **real data** from `get_user_subscription()` and `get_user_orders()` RPCs on tab activation. **No hardcoded fake data** — subscription status, renewal dates, billing period, and payment history all come from DB. Shows provider-specific icon + label: PayPal SVG icon for PayPal subs, Stripe "S" SVG for Stripe subs. **Provider-aware cancel flow**: Cancel button dispatches to correct provider endpoint based on `subscription.provider` — either `/api/payments/paypal/cancel-subscription` or `/api/payments/stripe/cancel-subscription`.
...
- Billing note text dynamically shows provider name ("Stripe" / "PayPal" / "our payment partners").
...
NEXT_PUBLIC_SITE_URL=          # Base URL for Stripe redirect URLs (e.g. https://yourdomain.com)
...
- **Stripe SDK**: `stripe` npm package v20.x. Client is **lazily initialized** in `lib/payments/stripe.ts` via `getStripeClient()` — avoids build-time crashes when env vars aren't set.
- **Checkout flow**: Creates a Stripe Checkout Session server-side, redirects user to Stripe's hosted page. On success, Stripe redirects to `/api/payments/stripe/success?session_id=...` which verifies and creates DB records.
- **Webhook events handled**: `checkout.session.completed` (fallback), `invoice.paid` (renewals + initial), `invoice.payment_failed`, `customer.subscription.updated`, `customer.subscription.deleted`.
- **Belt-and-suspenders**: Both the success redirect route and webhook create subscription records. Idempotency keys prevent duplicates.
- **Stripe SDK v20 notes**: `current_period_start`/`current_period_end` are on `subscription.items.data[0]`, not on the subscription object directly. Invoice `subscription` field is under `invoice.parent.subscription_details.subscription`.
- **Checkout page UI**: Tabbed payment selector (Card / PayPal) with Stripe as default. Tab only shows if both providers are configured for the plan. Stripe button redirects to hosted checkout; PayPal uses in-page SDK buttons.

### Setup Guide
See `PAYMENT-SETUP-GUIDE.md` for complete step-by-step instructions (PayPal + Stripe).

---

## ⚖️ Legal Pages (`/legals`)
- **Hub page** (`/legals`): Card grid linking to all legal documents with icons, titles, descriptions.
- **Terms & Conditions** (`/legals/terms-and-conditions`): Account terms, subscription services, user conduct, content usage rights, intellectual property, payments, termination, disclaimers.
- **Privacy Policy** (`/legals/privacy-policy`): Data collection, usage, sharing, security, retention, user rights, cookies, children's privacy.
- **Refund Policy** (`/legals/refund-policy`): No-refunds policy for digital content, subscription cancellation details, exceptions, chargebacks.
- **License Agreement** (`/legals/license`): Build licensing terms — personal use, content creation with credit, private commission rules (significant modifications required, not primary focus), prohibited uses (redistribution, resale, ownership claims, account sharing).

### Key License Terms
- Builds licensed under KnightyBuilds, not sold
- Cannot be resold, redistributed, or uploaded elsewhere
- No claiming ownership
- Videos/public media (YouTube, TikTok, etc.) allowed WITH credit to KnightyBuilds
- Private commissions allowed ONLY if: significant modifications made AND asset is not the primary focus
- Sharing subscription access is prohibited
- Personal modifications allowed, but modified versions cannot be redistributed or sold

### Footer Integration
- Legal links column added to Footer component: TERMS, PRIVACY, REFUNDS, LICENSE
- CSS module: `app/(public)/legals/page.module.css` (shared by all legal pages)
