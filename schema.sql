...
-- Record a product download (authenticated users only)
...
begin
  ...
  -- Record download (allow multiple downloads from same user)
  insert into public.product_downloads (product_id, user_id)
  values (p_product_id, v_user_id);

  return json_build_object('status', 'success', 'message', 'Download recorded');
end;
$$;

revoke all on function public.record_product_download(uuid) from public;
grant execute on function public.record_product_download(uuid) to authenticated;
...
-- ============================================
-- USER PREFERENCES & ACCOUNT DELETION RPCs
-- ============================================
...
-- Admin: get user newsletter preferences
...
begin
  ...
  from (
    select
      up.id, up.handle, up.display_name, up.avatar_url, up.tier,
      au.email, up.email_notifications, up.newsletter_opt_in, up.created_at
    from public.user_profiles up
    join auth.users au on au.id = up.id
    where (p_search = '' or up.handle ilike '%' || p_search || '%'
      or up.display_name ilike '%' || p_search || '%'
...
-- build-images: public read, admin write
...
-- build-downloads: authenticated read, admin write
