...
const TIER_ICONS: Record<string, string> = {
  explorer: '/icons/stone.png',
  access: '/icons/gold.png',
  builder: '/icons/diamond.png',
  architect: '/icons/netherite.png',
}
...
export default function PricingPage() {
  ...
  const jsonLd = {
    ...
    '@type': 'Product',
    name: 'KnightyBuilds Subscription',
    description: 'Premium Minecraft build subscriptions with unlimited downloads',
    brand: {
      '@type': 'Brand',
      name: 'KnightyBuilds'
    },
    offers: (plans || []).filter(p => p.monthlyPrice > 0).map(plan => ({
      '@type': 'Offer',
      name: plan.name,
      ...
      url: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://knightybuilds.com'}/pricing`,
    }))
  }
      ...
      <section className={styles.hero} ref={heroRef}>
        ...
        <div className={styles.heroContent}>
          ...
          <h1 className={styles.heroTitle}>
            UNLIMITED ACCESS.
            <br />
            <span className={styles.heroTitleAccent}>ONE SUBSCRIPTION</span>
          </h1>
          <p className={styles.heroSubtitle}>
            Instant downloads, step-by-step guides, and exclusive builds added regularly.
          </p>
            ...
            <div className={styles.toggleTrack}>
              ...
              >
                ...
                <span className={styles.saveBadge}>
                  {plans && plans.length > 0 && plans.some(p => p.monthlyPrice > 0)
                    ? `Save ${Math.round((1 - (plans.find(p => p.monthlyPrice > 0)?.yearlyPrice || 0) / ((plans.find(p => p.monthlyPrice > 0)?.monthlyPrice || 1) * 12)) * 100)}%`
                    : 'Save'}
                </span>
              </button>
            </div>
                ...
                <div className={styles.cardShowcase}>
                  ...
                  <div className={styles.cardShowcaseName}>
                    <div className={styles.cardIcon} style={{ background: accent }}>
                      <Image 
                        src={TIER_ICONS[plan.tier] || '/icons/stone.png'} 
                        alt="" 
                        width={24} 
                        height={24} 
                  ...
                  <div className={styles.cardPricing}>
                    {plan.monthlyPrice === 0 ? (
                      ...
                      <>
                        ...
                        <span className={styles.priceAmount}>{(plan.yearlyPrice / 12).toFixed(0)}</span>
