...
// Common swear words filter
const BLOCKED_WORDS = ['fuck', 'shit', 'ass', 'damn', 'bitch', 'cunt', 'dick', 'cock', 'pussy', 'nigger', 'faggot', 'retard', 'whore', 'slut']
...
export default function SettingsPage() {
  ...
  const [handleConfirmModalOpen, setHandleConfirmModalOpen] = useState(false)
  ...
  const handleCancelSubscription = async () => {
    ...
    // Route to correct provider's cancel endpoint
    const cancelUrl = subscription.provider === 'stripe'
  ...
  const handlePreferenceChange = async (field: 'email_notifications' | 'newsletter_opt_in', value: boolean) => {
    if (field === 'email_notifications') setEmailNotifications(value)
    else setNewsletterOptIn(value)
    setPrefSaving(true)

    try {
      const supabase = getSupabaseBrowserClient()
      const { data } = await supabase.rpc('update_user_preferences', {
        p_email_notifications: field === 'email_notifications' ? value : null,
        p_newsletter_opt_in: field === 'newsletter_opt_in' ? value : null,
      })
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (data?.status === 'success') {
        ...
        setMessage({ type: 'error', text: data?.message || 'Failed to request deletion' })
  ...
  const handleSaveProfile = async () => {
    ...
    if (formData.bio.length > 500) {
      setMessage({ type: 'error', text: 'Bio must be 500 characters or less' })
      return
    }
    
    // Check for blocked words
    if (containsBlockedWord(formData.display_name) || containsBlockedWord(formData.bio)) {
      setMessage({ type: 'error', text: 'Your profile contains inappropriate language. Please remove it and try again.' })
      return
    }
    ...
    try {
      ...
      const { error } = await supabase
        .from('user_profiles')
        .update({
          display_name: formData.display_name.slice(0, 20),
  ...
  const initiateHandleSave = () => {
    if (!handleChange.newHandle.trim()) return
    
    // Check if it's the same as current handle
    if (handleChange.newHandle.trim().toLowerCase() === profile?.handle?.toLowerCase()) {
      setHandleChange(prev => ({ ...prev, error: 'This is already your current handle' }))
      return
    }
    
    // Open confirmation modal
    setHandleConfirmModalOpen(true)
  }
  
  const handleSaveHandle = async () => {
    if (!user || !handleChange.newHandle.trim()) return
    
    setHandleConfirmModalOpen(false)
  ...
  return (
    <main className={styles.settings}>
                      ...
                      {isEditing ? (
                        <div className={styles.inputWithCounter}>
                          <input
                            ...
                            onChange={(e) => setFormData(prev => ({ ...prev, display_name: e.target.value.slice(0, 20) }))}
                            maxLength={20}
                          />
                          <span className={`${styles.charCounter} ${formData.display_name.length >= 20 ? styles.charCounterLimit : ''}`}>
                        ...
                        <div className={styles.inputWithCounter}>
                          <textarea
                            ...
                            onChange={(e) => {
                              ...
                              setFormData(prev => ({ ...prev, bio: val.slice(0, 300) }))
                            }}
                            rows={3}
                            maxLength={300}
                            style={{ maxHeight: '120px', resize: 'none' }}
                          />
                          <span className={`${styles.charCounter} ${formData.bio.length >= 270 ? styles.charCounterLimit : ''}`}>
              ...
              <div className={styles.panelBody}>
                {profile?.deletion_requested_at && (
                  <div className={styles.deletionBanner}>
                    <AlertCircle size={20} />
                    <div className={styles.deletionBannerContent}>
                      <p>
                        <strong>Account scheduled for deletion.</strong>{' '}
                        Your account will be manually removed soon. If you changed your mind, you can cancel this request.
                      </p>
                    </div>
                    <button
                      className={styles.deletionCancelBtn}
                      onClick={async () => {
                        const supabase = getSupabaseBrowserClient()
                    ...
                    <div className={styles.preferenceItem}>
                      ...
                      <label className={styles.toggle}>
                        <input
                          ...
                          disabled={prefSaving}
                          onChange={(e) => handlePreferenceChange('newsletter_opt_in', e.target.checked)}
                        />
                        <span className={styles.toggleSlider} />
                      </label>
                    </div>
                ...
                <div className={styles.dangerZone}>
                  ...
                  <div className={styles.dangerItem}>
                    ...
                    >
                      {profile?.deletion_requested_at ? 'Deletion Pending' : 'Delete Account'}
                    </button>
                  </div>
                </div>
              </div>
                ...
                {billingLoading ? (
                  ...
                  <>
                          ...
                          <div className={styles.paymentCard}>
                            ...
                            <div className={styles.paymentDetails}>
                              <span className={styles.paymentProvider}>
                                {subscription.provider === 'paypal' ? 'PayPal' : subscription.provider === 'stripe' ? 'Stripe (Card)' : 'Credit Card'}
                              </span>
                              <span className={styles.paymentHint}>
                                Managed via {subscription.provider === 'paypal' ? 'your PayPal account' : subscription.provider === 'stripe' ? 'Stripe' : 'payment provider'}
                              </span>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className={styles.billingNote}>
                      <AlertCircle size={16} />
                      <p>
                        Payments are processed securely via {subscription?.provider === 'stripe' ? 'Stripe' : subscription?.provider === 'paypal' ? 'PayPal' : 'our payment partners'}. We never store your payment details.
      ...
      {/* Handle Change Confirmation Modal */}
      {handleConfirmModalOpen && (
        <div className={styles.modalOverlay} onClick={() => setHandleConfirmModalOpen(false)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <AlertCircle size={24} className={styles.modalIconWarning} />
              <h3>Confirm Username Change</h3>
            </div>
            <div className={styles.modalBody}>
              <p>You are about to change your username to <strong>@{handleChange.newHandle}</strong>.</p>
              <p className={styles.modalWarning}>
                <strong>Important:</strong> After changing your username, you will not be able to change it again for <strong>14 days</strong>. 
            ...
            <div className={styles.modalActions}>
              <button 
                ...
                onClick={() => setHandleConfirmModalOpen(false)}
              >
                Cancel
              </button>
              <button 
                ...
                disabled={handleChange.saving}
              >
                {handleChange.saving ? 'Saving...' : 'Confirm Change'}
              </button>
            </div>
          </div>
        </div>
      ...
      {deleteModalOpen && (
        <div className={styles.modalOverlay} onClick={() => !deleteLoading && setDeleteModalOpen(false)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <Trash2 size={24} className={styles.modalIconDanger} />
              <h3>Delete Your Account</h3>
            </div>
            <div className={styles.modalBody}>
              <p>You are about to request deletion of your account. This will:</p>
              <ul className={styles.modalList}>
                <li>Schedule your account for permanent deletion</li>
                <li>Sign you out immediately</li>
                <li>Remove all your data after admin review</li>
              </ul>
              <p className={styles.modalNote}>
                <strong>Changed your mind?</strong> If you log back in before the deletion is processed, 
                you can cancel this request from your account settings.
              </p>
            </div>
            <div className={styles.modalActions}>
              <button 
