...
export default function ProductDetailPage() {
  ...
  const handleDownload = async () => {
    ...
    if (product?.download_url) {
      ...
      try {
        await supabase.rpc('record_product_download', {
          p_product_id: product.id
        })
      } catch (err) {
  ...
  const jsonLd = {
    ...
    url: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://knightybuilds.com'}/builds/${product.slug}`,
    author: product.publisher ? {
      '@type': 'Person',
      name: product.publisher.display_name,
      url: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://knightybuilds.com'}/${product.publisher.handle}`
    } : undefined,
    datePublished: product.created_at,
    keywords: product.tags?.join(', '),
    genre: product.theme_category,
    additionalType: product.build_type,
  }
