...
export default function Admin() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [statsRes, activityRes, deletionRes] = await Promise.all([
          supabase.rpc('admin_get_dashboard_stats'),
          supabase.rpc('admin_get_recent_activity', { p_limit: 5 }),
          supabase.rpc('admin_get_deletion_requests')
        ])
        ...
        if (deletionRes.data?.status === 'success') {
          setDeletionCount(deletionRes.data.total || 0)
        }
      } catch (err) {
  ...
  return (
    <div className={styles.dashboard}>
      ...
      {deletionCount > 0 && (
        <a href="/admin/deletion-requests" className={styles.alertBanner}>
          <AlertCircle size={18} />
          <span>
            <strong>{deletionCount} account deletion request{deletionCount !== 1 ? 's' : ''}</strong> pending review
          </span>
          <UserX size={16} style={{ marginLeft: 'auto', opacity: 0.5 }} />
        </a>
      )}
