...
values
  ...
  ('build-downloads', 'build-downloads', true, 52428800, array['application/zip', 'application/x-rar-compressed', 'application/x-7z-compressed', 'application/octet-stream'])
...
-- Storage policies for build-downloads (public read, tier check via app, admin write)
create policy "Public read access to build downloads"
  on storage.objects for select
  to anon, authenticated
